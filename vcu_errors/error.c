...
static ERROR_HANDLER_STATUS_T _Error_Handle_Timeout(ERROR_STATUS_T *er_stat,
  ...
  if (er_stat->error == false) {
    ...
    if (HAL_GetTick() - er_stat->time_stamp < timeout_ms) {
      er_stat->handling = true;
      return FINE;
    } else {
      return FAULT;
...
static ERROR_HANDLER_STATUS_T _Error_Handle_Last_Timestamp_Timeout(ERROR_STATUS_T *er_stat,
                                                    uint32_t        timeout_ms) {
  // Board_Println("Call to Handle Timeout");
  if (er_stat->error == false) {
    er_stat->handling = false;
    ...
    // [TODO] magic numbers change
    if (HAL_GetTick() - er_stat->last_time_stamp < timeout_ms) {
      er_stat->handling = true;
      return FINE;
    } else {
      return FAULT;
...
static ERROR_HANDLER_STATUS_T _Error_Handle_Immediate(ERROR_STATUS_T *er_stat, uint32_t condition) {
  UNUSED(condition);
...
static ERROR_HANDLER error_handler_vector[ERROR_NUM_ERRORS] = {
  { _Error_Handle_Immediate, GATE_RECOVERABLE_TIMEOUT },
  { _Error_Handle_Timeout, GATE_FATAL_TIMEOUT     },
  { _Error_Handle_Timeout, PRECHARGE_TIMEOUT     },
  { _Error_Handle_Timeout, L_CONTACTOR_MISMATCH_TIMEOUT     },
  { _Error_Handle_Timeout, R_CONTACTOR_MISMATCH_TIMEOUT      },
  { _Error_Handle_Immediate, BRAKE_ACCEL_TIMEOUT },
  { _Error_Handle_Last_Timestamp_Timeout, ACCEL_CONFLICT_TIMEOUT   },
  { _Error_Handle_Timeout, BMS_HEARTBEAT_BAD_TIMEOUT },
  { _Error_Handle_Timeout, FCN_HEARTBEAT_BAD_TIMEOUT },
  { _Error_Handle_Timeout, MC_HEARTBEAT_BAD_TIMEOUT },
  { _Error_Handle_Timeout, CSB_HEARTBEAT_BAD_TIMEOUT },
...
void Error_Init(void) {
  ...
  for (i = 0; i < ERROR_NUM_ERRORS; ++i) {
    ...
    error_vector[i].last_time_stamp = 0;
...
void Error_Present(ERROR_T er_t) {
  ...
  if (!error_vector[er_t].error) {
    ...
    error_vector[er_t].time_stamp = HAL_GetTick();
    error_vector[er_t].last_time_stamp = error_vector[er_t].time_stamp;
    error_vector[er_t].count      = 1;
  }
  else {
    error_vector[er_t].last_time_stamp = HAL_GetTick();
    error_vector[er_t].count += 1;
  }
}
...
bool Check_Error(ERROR_T er_t, bool Force_Check) {
  if (Error_Care(er_t) || Force_Check) {
    return error_handler_vector[er_t].handler(&error_vector[er_t], error_handler_vector[er_t].condition) == FAULT;
  }
  else {
    return false;
  }
...
bool Check_Error_Group(ERROR_T* er_t) {
  for(uint8_t i = 0; i<sizeof(er_t)/er_t[0], i++) {
    if (Check_Error(er_t[i], false)) {
      return true;
    }
  }
  return false;
